...
object ex42 extends App {
  ...
  val testi = pages.join(ranks)
  val testi2 = pages.join(ranks).flatMap(l => l._2._1.map(b => "a"))
  testi.foreach(println)
  testi2.foreach(println)
  // Run 10 iterations of PageRank
  for (i <- 0 until 10) {
    
    val contributions: RDD[(String, Double)] = pages.join(ranks).flatMap (l => l._2._1.map(x => (x, l._2._2/l._2._1.size)))
  ...
  // Task #2: Would you solution benefit from caching the data? If yes, how?
  ...
  val invertedIndex: Map[String, Iterable[(Int, Int)]] = index.flatMap(l => l._2.map(e => (e,(l._1, 1))))
                                                          .groupByKey()
                                                          .collect
                                                          .toMap
  ...
  val voila = arr.zipWithIndex.map{ case (s,i) => (s,i) } // = Array(("One", 0), ("Two", 1), ("Three", 2))
  
  val proximityIndex: Map[String, Iterable[(Int, Int)]] = index.flatMap(v => {v._2.zipWithIndex.map{
    case(hit, count) => (hit, (v._1, count)) 
